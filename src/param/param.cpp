...
void keksUpdate()
{
    std::cout << "Now: " << keks << std::endl;
}
PARAM_ADD_WITH_CALLBACK(param::FLOAT, TEST1_testVariable, &keks, keksUpdate)
PARAM_ADD(param::FLOAT, TEST1_testVariable2, &keks)
PARAM_ADD(param::FLOAT, TEST1_testVariable3, &keks)

PARAM_ADD(param::FLOAT, TEST5_testVariable, &keks)
PARAM_ADD(param::FLOAT, TEST5_testVariable2, &keks)
PARAM_ADD(param::FLOAT, TEST5_testVariable3, &keks)

PARAM_ADD(param::FLOAT, _TEST_testVariable, &keks)
PARAM_ADD(param::FLOAT, _TEST_testVariable2, &keks)
PARAM_ADD(param::FLOAT, _TEST_testVariable3, &keks)

unsigned param::getParamCount() { return ((&__param_stop - &__param_start) - 1); }

bool param::getParamByName(const char name[], param::paramVarId_t &param)
{
    unsigned L = 0;
    unsigned R = (&__param_stop - &__param_start) - 2;
    while (L <= R)
    {
        unsigned m = (L + R) / 2;
...
{
    if (index > ((&__param_stop - &__param_start) - 1))
    {
        param.ptr = nullptr;
        param.index = UINT16_MAX;
        return false;
...
{
    if (param > ((&__param_stop) - 1) or param < &__param_start)
        return false;
    if (param->type > _end)
        return false;
