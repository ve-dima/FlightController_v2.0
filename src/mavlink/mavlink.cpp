...
void mavlink_state_report(mavlink_channel_t ch)
{
    ...
    const Eigen::Quaternionf attitude = AHRS::getFRD_Attitude();
    const float attitudeArray[4] = {attitude.w(), attitude.x(), attitude.y(), attitude.z()};
    const Eigen::Vector3f rotateRate = AHRS::getFRD_RotateSpeed();
...
void mavlink_battery_report(mavlink_channel_t ch)
{
    const uint16_t cellVoltage[10] = {static_cast<uint16_t>(Battery::getVoltage() * 1e3),
                                      UINT16_MAX,
                                      UINT16_MAX,
                                      UINT16_MAX,
                                      UINT16_MAX,
                                      ...
                                      UINT16_MAX,
                                      UINT16_MAX};
    const uint16_t cellVoltageExt[4] = {
        UINT16_MAX,
        UINT16_MAX,
        UINT16_MAX,
        UINT16_MAX,
    ...
    const int8_t battery_remaining = Battery::getPercent();

    mavlink_msg_battery_status_send(ch,
                                    1, MAV_BATTERY_FUNCTION::MAV_BATTERY_FUNCTION_ALL, MAV_BATTERY_TYPE::MAV_BATTERY_TYPE_LIPO,
                                    -1,
                                    cellVoltage,
                                    -1,
                                    -1,
                                    ...
                                    battery_remaining,
                                    -1,
                                    MAV_BATTERY_CHARGE_STATE::MAV_BATTERY_CHARGE_STATE_UNDEFINED, cellVoltageExt, MAV_BATTERY_MODE::MAV_BATTERY_MODE_UNKNOWN,
                                    0);
}
...
void handler()
{
    for (static uint32_t lowFreqTim = 0; millis() - lowFreqTim > 1'000; lowFreqTim = millis())
    {
        mavlink_heartbeat_report(MAVLINK_COMM_0), mavlink_heartbeat_report(MAVLINK_COMM_1);
        mavlink_pressure_report(MAVLINK_COMM_0), mavlink_pressure_report(MAVLINK_COMM_1);
        mavlink_battery_report(MAVLINK_COMM_0), mavlink_battery_report(MAVLINK_COMM_1);
    }

    for (static uint32_t mediumFreqTim = 0; millis() - mediumFreqTim > 250; mediumFreqTim = millis())
    {
